...
figure();
plotHandler = plot(dataHandlerFitted.inputSeq, dataHandlerFitted.outputSeq,...
    '-b','linewidth',lineWidth,...
    'DisplayName','Miller model'); hold on;
xlim([-2000 2000])
ylim([-40 75])
xticks([-2000 -1400 -700 0 700 1400 2000])
% yticks([-600 -300 0 300 600 900 1200])
xlabel('$V$','Interpreter','latex')
ylabel("$P\ \ [\frac{\mu C}{{cm}^2}]$",'Interpreter','latex')

uSat = linspace(-2500,2500,50);
plot(uSat,PsatPlus(uSat*inputInvScale)/outputInvScale,...
    '--r',...    'Color',[150,0,0]/255,...
    'LineWidth',1.2,...
    'DisplayName','$P_{sat}^+$',...
    'HandleVisibility','on');
plot(uSat,PsatMinus(uSat*inputInvScale)/outputInvScale,...
    '--b',...    'Color',[0,0,120]/255,...
    'LineWidth',1.2,...
    'DisplayName','$P_{sat}^-$',...
    'HandleVisibility','on');
